...
import { Search, Plus, MoreHorizontal, Filter, MessageSquareCode, X, Edit, Trash2 } from 'lucide-react';
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium">Administre o corpo discente com facilidade.</p>
        </div>
        <button 
      ...
      {aiInsight && (
        <div className="bg-indigo-600 text-white p-6 rounded-3xl flex items-start gap-4 shadow-2xl relative animate-fade-in">
          <div className="bg-white/20 p-3 rounded-2xl"><MessageSquareCode size={24} /></div>
        ...
        <div className="p-6 border-b border-slate-50 flex items-center gap-4">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-12 pr-6 py-3.5 bg-slate-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-indigo-500 outline-none"
              value={searchTerm}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 text-slate-400 text-[11px] font-black uppercase tracking-widest">
                <th className="px-8 py-5">Aluno / Idade</th>
                <th className="px-8 py-5">Série / Escola</th>
                <th className="px-8 py-5">Disciplinas</th>
                ...
                <tr key={student.id} className="hover:bg-slate-50/80 group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-indigo-600 text-white rounded-2xl flex items-center justify-center font-black text-lg shadow-md">
                  ...
                  <td className="px-8 py-6">
                    ...
                    <p className="text-xs font-medium text-slate-400">{student.school}</p>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex flex-wrap gap-1">
                      {student.subjects?.map((sub, i) => (
                        <span key={i} className="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-lg text-[10px] font-black uppercase">
                          {sub}
                        </span>
                      ))}
                    </div>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden border border-white/20">
            <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-indigo-600 text-white">
              <div>
                ...
                <p className="text-indigo-100 text-xs font-bold uppercase tracking-widest mt-1">Sincronização em Tempo Real</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-4">
              ...
              <div className="pt-6 flex gap-4">
                ...
                <button type="submit" disabled={isSubmitting} className="flex-1 px-6 py-4 bg-indigo-600 text-white rounded-2xl text-sm font-black hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all active:scale-95 disabled:opacity-50">
