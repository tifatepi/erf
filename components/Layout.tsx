...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, user, onLogout }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      setIsMobile(mobile);
      // Se não for mobile, mantém a sidebar aberta por padrão
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      {isMobile && isSidebarOpen && (
        <div 
          className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] transition-opacity animate-in fade-in"
