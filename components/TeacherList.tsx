...
import { Search, Plus, X, Edit, Trash2, User, GraduationCap, Phone, Hash } from 'lucide-react';
import { Teacher } from '../types';

interface TeacherListProps {
  teachers: Teacher[];
  onAdd: (data: Partial<Teacher>) => Promise<void>;
  onUpdate: (id: string, data: Partial<Teacher>) => Promise<void>;
  onDelete: (id: string) => Promise<void>;
}

const TeacherList: React.FC<TeacherListProps> = ({ teachers, onAdd, onUpdate, onDelete }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', cpf: '', education: '', phone: '' });
    } catch (err) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tight">Corpo Docente</h2>
          <p className="text-slate-500 font-medium">Gerenciamento de professores e instrutores.</p>
        </div>
        <button 
          onClick={openAdd}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 text-slate-400 text-[11px] font-black uppercase tracking-widest">
                <th className="px-8 py-5">Docente / CPF</th>
                ...
                <tr key={t.id} className="hover:bg-slate-50/80 transition-all group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-black tracking-tight">{t.cpf || 'Sem CPF'}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => openEdit(t)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all">
                        <Edit size={16} />
                      </button>
                      <button onClick={() => onDelete(t.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 animate-fade-in">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg border border-white/20">
            <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-indigo-600 text-white rounded-t-[2.5rem]">
              <div>
                <h3 className="font-black text-2xl tracking-tight">{editingTeacher ? 'Editar Docente' : 'Novo Docente'}</h3>
                <p className="text-indigo-100 text-xs font-bold uppercase tracking-widest mt-1">Dados Profissionais</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">CPF</label>
                <div className="relative">
              ...
              <div className="space-y-1">
                <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Formação Acadêmica</label>
                <div className="relative">
